...
if __name__ == '__main__':
    ...
    if args.train is True:
        train(dataset_path_json=os.path.join(args.data_path, JSON_FILE_PATH), input_shape_image=INPUT_SHAPE_IMAGE)

    if parse_args().train_dif_shape is True:
        for i in range(5):
            input_shape = [(INPUT_SHAPE_IMAGE[0] + (64 * i), INPUT_SHAPE_IMAGE[1] +
                            (64 * i), 3) for i in range(0, 5, 1)]
            train(dataset_path_json=os.path.join(args.data_path, JSON_FILE_PATH), input_shape_image=input_shape[i])
